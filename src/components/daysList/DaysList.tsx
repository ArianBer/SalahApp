...
export const DaysList = ({ onDateSelection }: DayListTabs) => {
  ...
  const offset = todayIndex * itemWidth - (Dimensions.get('window').width - itemWidth) / 2;
  const [selectedDate, setSelectedDate] = useState(today);
  const handlePress = (date: Date) => () => {onDateSelection(date); setSelectedDate(date)};

  return (
    <ScrollView horizontal contentOffset={{ x: offset, y: 0 }} showsHorizontalScrollIndicator={false}>
      {days.map((date) => (
        <TouchableOpacity
